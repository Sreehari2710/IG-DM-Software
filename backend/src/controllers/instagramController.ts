...
export const connectInstagram = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'production') {
      (launchOptions as Record<string, unknown>).channel = 'chrome';
    }
    ...
    try {
      ...
      if (launchOptions && 'channel' in (launchOptions as any)) {
        delete (launchOptions as any).channel;
      }
      try {
        context = await chromium.launchPersistentContext(profileDir, launchOptions);
      } catch (innerErr: any) {
    ...
    while (Date.now() - startTime < 180_000) {
      ...
      try {
        ...
        const hasSession = cookies.some((c: any) => c.name === 'sessionid');
    ...
    try {
      ...
      const handle = await page.$eval('input[name="username"]', (el: Element) => (el as HTMLInputElement).value).catch(() => '');
      if (handle) {
        instagramUsername = handle;
      } else {
        const cookies = await context.cookies();
        const dsUser = cookies.find((c: any) => c.name === 'ds_user_id');
        if (dsUser) instagramUsername = `user_${dsUser.value}`;
      }
    } catch {
      const cookies = await context.cookies();
      const dsUser = cookies.find((c: any) => c.name === 'ds_user_id');
...
export const updateSessionProxy = async (req: Request, res: Response): Promise<void> => {
